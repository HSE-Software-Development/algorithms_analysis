...
void FordBellman(size_t startIndex, GeneralGraph<T, K> *graph,
...
/// @param distances vector of distances from start point to all others
template <typename T, typename K>
void DFS_Rec(const GeneralNode<T> *node, const GeneralGraph<T, K> *graph,
             std::vector<T> &distances) {
  for (const std::pair<GeneralNode<T> *, size_t> &node_and_index :
    ...
    if (distances[node_and_index.first->getIndex()] == inf) {
      distances[node_and_index.first->getIndex()] =
          ...
          graph->getEdge(node_and_index.second)->getWeight();
...
/// @param distances vector of distances from start point to all others
template <typename T, typename K>
void DFS(size_t startIndex, GeneralGraph<T, K> *graph,
         std::vector<T> &distances) {
  const auto size = graph->getSize();
...
/// @param distances vector of distances from start point to all others
template <typename T, typename K>
void BFS(size_t startIndex, GeneralGraph<T, K> *graph,
         std::vector<T> &distances) {
  const auto size = graph->getSize();
...
/// @param distances vector of distances from start point to all others
template <typename T, typename K>
void BFS_WithZeroEdges(size_t startIndex, GeneralGraph<T, K> *graph,
                       std::vector<T> &distances) {
  const auto size = graph->getSize();
...
/// @param distances vector of distances from start point to all others
template <typename T, typename K>
void Dijkstra(size_t startIndex, GeneralGraph<T, K> *graph,
              std::vector<T> &distances) {
  const auto size = graph->getSize();
  ...
  for (int i = 0; i < size; i++) {
    ...
    a[v] = true;
    for (const std::pair<GeneralNode<T> *, size_t> &node_and_index :
         graph->getNode(v)->getNeighbours()) {
      distances[node_and_index.first->getIndex()] = std::min(
          distances[node_and_index.first->getIndex()],
